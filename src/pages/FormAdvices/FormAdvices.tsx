...
import { checkPrhase, getAdviceByIdService, getAuthorsSelectApi } from "../../services/advices-service";
...
export default function FormAdvices({mode}:{mode:string}){
    ...
    async function submit(info:upsertAdviceBody){
     ...
     if (loadingAuthor || loadingListAuthors) return <div className="loading-screen">Cargando...</div>;
    return(
        <>
  ...
  <div className="form-card p-4">
    <h2 id="subtitle" className="mb-4 text-center homenaje-regular">
      {mode == "create" ? "Crear Consejo" : mode=="edit" ? "Editar consejo" : "Eliminar consejo"}
    </h2>
    
    <form action="" onSubmit={handleSubmit(submit)}>
      <div className="mb-3">
        <label className="d-block mb-2"><strong><i>Frase</i></strong></label>
        <textarea 
          rows={3} 
          className={`form-control ${errors.phrase ? "is-invalid" : ""}`}
          disabled={mode=="delete"} 
          defaultValue={mode=="edit" || mode=="delete" ? advice.phrase : ""}
          {...register("phrase", {
            required:{ value : mode!="delete", message: "La frase es obligatoria"},
                        validate : async (value) => {
                          let response = null;
                          try{
                            response = await checkPrhase(value);
                            return !response.existsPrhase || "La frase ya existe en la base de datos"
                          }catch(e){
      ...
      <div className="mb-3">
        <label className="d-block mb-2"><strong><i>Fecha</i></strong></label>
        <input 
        type="date"
          className={`form-control ${errors.date ? "is-invalid" : ""}`}
          disabled={mode=="delete"} 
          defaultValue={mode=="edit" || mode=="delete" ? advice.date : ""}
            {...register("date", {
...
<div className="mb-3">
        <label className="d-block mb-2"><strong><i>Author</i></strong></label>
        
        <select
        className={`form-control ${errors.idAuthor ? "is-invalid" : ""}`}
          disabled={mode=="delete"}
          defaultValue={mode=="edit" || mode=="delete" ? advice.idAuthor : ""} 
        {...register("idAuthor",{
          required:"El author es requerido",
          valueAsNumber:true
        })}>
          {listAuthors.map(author => <option  key={author.idAuthor} value={author.idAuthor} >{author.fullName}</option>)}
        </select>
        
               {errors.idAuthor && (
          <div className="invalid-feedback">{errors.idAuthor.message}</div>
        )}
      </div>
